...
def generate_blogs_report(data) -> pd.DataFrame:
    ...
    data_cols = [ 
        'id', 'user', 'title', 'content', 'tags', 'media_file', 'created_at'
    ]
...
def generate_comments_report(data) -> pd.DataFrame:
    ...
    data_cols = [
        'id', 'title', 'comments_count', 'comment'
    ]
...
def return_csv_response(df: pd.DataFrame, namescope:str) -> HttpResponse:
    ...
    response[
        'Content-Disposition'] = f'attachment; filename="{namescope}-{file_name}.csv"'
